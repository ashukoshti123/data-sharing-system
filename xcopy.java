...
class Data_Transfer extends Thread implements ActionListener
{
 static JFrame frame;
  ...
  static JCheckBox ec=new JCheckBox("Encoded Transfer");
  static JCheckBox dc=new JCheckBox("Decoded Transfer");
...
static int rate=1000,len;
  ...
  static String spath,dpath,spath2,joindest,key,emailid,orig_key="";


static InputStream in = null;
static  OutputStream out = null;
static   byte[] buf = new byte[rate];
  ...
  static String path=Login.mediapath;
...
//=============================================================================================================================================
   ...
   public static void System_Start()throws Exception
   {
	 ...
	 frame=new JFrame("Xtreme Copy");                         //create Frame with Title Xstream Copy
	 ...
	 Image imgs = Toolkit.getDefaultToolkit().getImage(xcopy.path+"ts.gif");  //  Create start button
		is=new ImageIcon(imgs);                                           //  assign image icon
	    bstart = new JButton("Start",is);                                         //  from image folder

	 Image imgp = Toolkit.getDefaultToolkit().getImage(xcopy.path+"tp.gif");
		ip=new ImageIcon(imgp);                                            // same for pause button
	    bpause = new JButton("Pause",ip);


	 Image imge = Toolkit.getDefaultToolkit().getImage(xcopy.path+"error.gif");  // same for stop button
	    ie=new ImageIcon(imge);
	    bstop = new JButton("Stop",ie);

	 Image imgs2 = Toolkit.getDefaultToolkit().getImage(xcopy.path+"ts2.gif");   // set rollover image of start button
		is2=new ImageIcon(imgs2);

	 Image imgp2 = Toolkit.getDefaultToolkit().getImage(xcopy.path+"tp2.gif");   // set rollover image of pause button
		ip2=new ImageIcon(imgp2);

	 Image imge2 = Toolkit.getDefaultToolkit().getImage(xcopy.path+"error2.gif"); // set rollover image of stop button
   	 ...
   	 Image imgx = Toolkit.getDefaultToolkit().getImage(xcopy.path+"frame_logo.gif");   //set frame logo to main frame
   ...
   public void actionPerformed(ActionEvent e)
	{
	  ...
	  if(o==mbtnhelp)
	  {
	    try
	    {
		  ...
		  run.exec("notepad "+xcopy.path+"help.txt");   // open help txt file
	    }
	    catch(Exception ee)
	    {
...
//=============================================================================================================================================
	...
	{
	  if(ec.isSelected() && !dc.isSelected())   // for encode
	  {
		 ce=1;
		 play_sound("Enter the pasword");
		 key=JOptionPane.showInputDialog(frame,"Enter the key for encryption ","Key",JOptionPane.QUESTION_MESSAGE);
		 emailid=JOptionPane.showInputDialog(frame,"Enter Receiver Email_ID:  ","Email_ID",JOptionPane.QUESTION_MESSAGE);
	  
		 EmailSender.mailsent(emailid,key);
	  }
	  else
	   if(!ec.isSelected() && dc.isSelected())  // for decode
	   {
 		 ce=2;
		 play_sound("Enter the pasword");
		 key=JOptionPane.showInputDialog(frame,"Enter the key for decryption ","Key",JOptionPane.QUESTION_MESSAGE);
	   }
	}
...
//=============================================================================================================================================
	  ...
	  try
	  {
		ac=Applet.newAudioClip(new File(xcopy.path+s+".wav").toURL());
		ac.play();
	  }
	  catch(Exception e){}
	}
